...
from keras.layers import GlobalAveragePooling2D,AveragePooling1D,MaxPooling1D, AveragePooling2D,GlobalAveragePooling1D
...
def build_model():
    model = Sequential()
    model.add(TimeDistributed(Conv2D(nb_filter=32, nb_row=1, nb_col=3, init='glorot_uniform',
                     ...
                     b_constraint=None, bias=True), input_shape=(None, 1, 1, 18)))

    model.add(TimeDistributed(AveragePooling2D(pool_size=(1, 2), strides=None, border_mode='valid', dim_ordering='th')))
    model.add(TimeDistributed(Conv2D(nb_filter=16, nb_row=1, nb_col=3, init='glorot_uniform',
                     activation='relu', weights=None, border_mode='valid',
                     subsample=(1, 1), dim_ordering='th', W_regularizer=None,
                     b_regularizer=None, activity_regularizer=None, W_constraint=None,
                     b_constraint=None, bias=True)))
    model.add(TimeDistributed(AveragePooling2D(pool_size=(1, 2), strides=None, border_mode='valid', dim_ordering='th')))
    model.add(TimeDistributed(Conv2D(nb_filter=8, nb_row=1, nb_col=3, init='glorot_uniform',
                     activation='relu', weights=None, border_mode='valid',
                     subsample=(1, 1), dim_ordering='th', W_regularizer=None,
                     b_regularizer=None, activity_regularizer=None, W_constraint=None,
                     b_constraint=None, bias=True)))
    ...
    model.summary()
    plot_model(model, to_file='cnn_lstm/model_plot_orig.png', show_shapes=True, show_layer_names=True)
    start = time.time()
...
def build_model_improved():
    ...
    model.add(GlobalAveragePooling1D(name="global_avg"))
    model.summary()
    plot_model(model, to_file='cnn_lstm/model_plot_improved.png', show_shapes=True, show_layer_names=True)
    start = time.time()
    model.compile(loss="mse", optimizer="adam")
    print("> Compilation Time : ", time.time() - start)
    return model
